...
async fn boot() -> (PageService, SearchService, mnemos::storage::AppState, tempfile::TempDir) {
...
async fn search_finds_keyword() {
    ...
    let fm = Frontmatter { title: Some("Kafka streaming".into()), ..Default::default() };
    pages.create(&user, "kafka", fm, "Kafka is a distributed event streaming platform.").await.unwrap();
...
async fn search_multi_word() {
    let (pages, search, state, _d) = boot().await;
    let user = ensure_user(&state).await;
    let fm = Frontmatter { title: Some("Postgres".into()), ..Default::default() };
    pages.create(&user, "pg", fm, "PostgreSQL is a relational database with JSON support.").await.unwrap();

    let hits = search.search(&user, "postgres json", 10).await.unwrap();
    assert!(!hits.is_empty(), "expected at least one hit for 'postgres json'");
}
...
async fn search_no_match_returns_empty() {
    ...
    pages.create(&user, "x", fm, "alpha beta gamma").await.unwrap();
...
async fn search_user_isolation() {
    ...
    let u1 = {
        let password_hash = mnemos::auth::hash_password("pw").unwrap();
        user_repo::insert(&state.db, "u1", &password_hash).await.unwrap().id
    };
    let u2 = {
        let password_hash = mnemos::auth::hash_password("pw").unwrap();
        user_repo::insert(&state.db, "u2", &password_hash).await.unwrap().id
    };
    let fm = Frontmatter { title: Some("Secret".into()), ..Default::default() };
    pages.create(&u1, "secret", fm, "this is a top-secret project codenamed phoenix").await.unwrap();
...
async fn search_ranking_prefers_exact_title_match() {
    let (pages, search, state, _d) = boot().await;
    let user = ensure_user(&state).await;
    let a = Frontmatter { title: Some("Cassandra".into()), ..Default::default() };
    let b = Frontmatter { title: Some("Cassandra vs Kafka".into()), ..Default::default() };
    pages.create(&user, "a", a, "Cassandra is a wide-column store.").await.unwrap();
    pages.create(&user, "b", b, "Both Cassandra and Kafka are popular in event-driven systems.").await.unwrap();
...
async fn search_honours_limit() {
    ...
    let user = ensure_user(&state).await;
    for i in 0..5 {
        let fm = Frontmatter { title: Some(format!("Doc {i}")), ..Default::default() };
        pages.create(&user, &format!("d{i}"), fm, "common keyword alpha beta").await.unwrap();
    }
...
async fn search_finds_tag_in_title_or_body() {
    let (pages, search, state, _d) = boot().await;
    let user = ensure_user(&state).await;
    let fm = Frontmatter { title: Some("Tagged".into()), tags: vec!["rust".into()], page_type: Some(PageType::Concept), ..Default::default() };
    pages.create(&user, "tagged", fm, "this is a rust page").await.unwrap();
