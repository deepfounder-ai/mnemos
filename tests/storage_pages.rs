...
async fn insert_and_get_by_id() {
    ...
    let fetched = page_repo::get_by_id(&state.db, &row.id)
        .await
        .unwrap()
        .unwrap();
...
async fn get_by_slug() {
    let (state, _d) = boot().await;
    let user = make_user(&state).await;
    page_repo::insert(&state.db, &user, "alpha", "Alpha", "{}", "a")
        .await
        .unwrap();
    let fetched = page_repo::get_by_slug(&state.db, &user, "alpha")
        .await
        .unwrap();
    assert!(fetched.is_some());
    assert_eq!(fetched.unwrap().title, "Alpha");
    let missing = page_repo::get_by_slug(&state.db, &user, "nope")
        .await
        .unwrap();
...
async fn unique_slug_per_user() {
    let (state, _d) = boot().await;
    let user = make_user(&state).await;
    page_repo::insert(&state.db, &user, "x", "X", "{}", "")
        .await
        .unwrap();
    // Same slug, same user -> conflict.
    let err = page_repo::insert(&state.db, &user, "x", "X2", "{}", "")
        .await
        .unwrap_err();
...
async fn slug_can_repeat_across_users() {
    ...
    let password_hash = mnemos::auth::hash_password("pw").unwrap();
    let u2 = user_repo::insert(&state.db, "u2", &password_hash)
        .await
        .unwrap()
        .id;
    page_repo::insert(&state.db, &u1, "shared", "Shared", "{}", "")
        .await
        .unwrap();
    page_repo::insert(&state.db, &u2, "shared", "Shared", "{}", "")
        .await
        .unwrap();
    let a = page_repo::get_by_slug(&state.db, &u1, "shared")
        .await
        .unwrap()
        .unwrap();
    let b = page_repo::get_by_slug(&state.db, &u2, "shared")
        .await
        .unwrap()
        .unwrap();
    assert_ne!(a.id, b.id);
...
async fn update_content_bumps_updated_at() {
    let (state, _d) = boot().await;
    let user = make_user(&state).await;
    let row = page_repo::insert(&state.db, &user, "k", "K", "{}", "old")
        .await
        .unwrap();
...
async fn delete_removes_row() {
    let (state, _d) = boot().await;
    let user = make_user(&state).await;
    let row = page_repo::insert(&state.db, &user, "k", "K", "{}", "")
        .await
        .unwrap();
    assert!(page_repo::delete(&state.db, &row.id).await.unwrap());
    assert!(page_repo::get_by_id(&state.db, &row.id)
        .await
        .unwrap()
        .is_none());
...
async fn list_for_user_orders_by_updated_at() {
    let (state, _d) = boot().await;
    let user = make_user(&state).await;
    page_repo::insert(&state.db, &user, "a", "A", "{}", "")
        .await
        .unwrap();
    tokio::time::sleep(std::time::Duration::from_millis(5)).await;
    page_repo::insert(&state.db, &user, "b", "B", "{}", "")
        .await
        .unwrap();
    tokio::time::sleep(std::time::Duration::from_millis(5)).await;
    let r = page_repo::insert(&state.db, &user, "c", "C", "{}", "")
        .await
        .unwrap();
    // Force a bump on `a` so it sorts to the top.
    tokio::time::sleep(std::time::Duration::from_millis(5)).await;
    page_repo::update_content(&state.db, &r.id, "C2", "{}", "")
        .await
        .unwrap(); // wrong, fix:
    let a_id = page_repo::get_by_slug(&state.db, &user, "a")
        .await
        .unwrap()
        .unwrap()
        .id;
    page_repo::update_content(&state.db, &a_id, "A2", "{}", "")
        .await
        .unwrap();
...
async fn cascade_delete_on_user_removal() {
    let (state, _d) = boot().await;
    let password_hash = mnemos::auth::hash_password("pw").unwrap();
    let user_id = user_repo::insert(&state.db, "tempo", &password_hash)
        .await
        .unwrap()
        .id;
    page_repo::insert(&state.db, &user_id, "p1", "P1", "{}", "")
        .await
        .unwrap();
    page_repo::insert(&state.db, &user_id, "p2", "P2", "{}", "")
        .await
        .unwrap();
    ...
    let slugs = page_repo::slugs_for_user(&state.db, &user_id)
        .await
        .unwrap();
    assert!(
        slugs.is_empty(),
        "expected pages to cascade-delete with user"
    );
}
