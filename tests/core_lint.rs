...
async fn boot() -> (PageService, SourceService, Linter, mnemos::storage::AppState, tempfile::TempDir) {
...
async fn empty_wiki_is_clean() {
    ...
    assert!(report.findings.is_empty(), "expected clean report, got: {:#?}", report.findings);
}
...
async fn detects_orphan_page() {
    ...
    let user = ensure_user(&state, "u").await;
    let fm = Frontmatter { title: Some("Orphan".into()), ..Default::default() };
    pages.create(&user, "orphan", fm, "no related links").await.unwrap();
...
async fn detects_broken_related() {
    let (pages, _s, linter, state, _d) = boot().await;
    let user = ensure_user(&state, "u").await;
    let fm = Frontmatter { title: Some("A".into()), related: vec!["nope".into()], ..Default::default() };
...
async fn detects_missing_source() {
    ...
    let fm = Frontmatter {
        ...
        sources: vec![SourceRef::url("sources/ghost-id-slug.md", "https://example.com")],
        ..Default::default()
    };
...
async fn happy_path_no_findings() {
    ...
    let src = sources.upload(&user, "notes", b"raw source content").await.unwrap();
    let mut fm_a = Frontmatter { title: Some("A".into()), related: vec!["b".into()], ..Default::default() };
    fm_a.sources = vec![SourceRef::url(
        format!("sources/{}-notes.md", src.source_id),
        "https://example.com",
    )];
    pages.create(&user, "a", fm_a, "a body").await.unwrap();
    let mut fm_b = Frontmatter { title: Some("B".into()), related: vec!["a".into()], ..Default::default() };
    ...
    for f in &report.findings {
        assert!(f.kind != FindingKind::BrokenRelated, "unexpected broken: {f:?}");
        assert!(f.kind != FindingKind::MissingSource, "unexpected missing: {f:?}");
...
async fn user_isolation_in_lint() {
    ...
    let fm = Frontmatter { title: Some("X".into()), related: vec!["ghost".into()], ..Default::default() };
