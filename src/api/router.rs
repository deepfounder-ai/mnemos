...
pub fn router(state: AppState) -> Router {
    ...
    Router::new()
        ...
        .fallback(not_found)
        .layer(
            // Per-request access logs at INFO so a running container is not
            // silent: one span (method + path) + one line on response with
            // status and latency. Bump MNEMOS_LOG=tower_http=debug for headers.
